...
public class InMemoryTrackDAO implements TrackDAO {
    ...
    public List<Track> getTracksByArtistName(String artistName) {
        ...
        return tracksMap.values()
                .stream()
                .filter(t -> t.getArtists().stream().anyMatch(artist -> artist.getName().equals(artistName))).collect(Collectors.toList());
...
//                .collect(Collectors.toList());
    ...
    public void init() {
        ...
        Track track1 = Track.builder().id(nextId++).title("Too Sweet").album("Unheard").issueDate(LocalDate.of(2024,3,22)).durationSec(251).audioType("MP3").artists(List.of(artist1)).build();
        Track track2 = Track.builder().id(nextId++).title("Like That").album("We Don't Trust You").issueDate(LocalDate.of(2024,3,26)).durationSec(267).audioType("MP3").artists(List.of(artist2,artist3)).build();
        Track track3 = Track.builder().id(nextId++).title("Beautiful Things").album("Fireworks & Rollerblades").issueDate(LocalDate.of(2024,1,18)).durationSec(180).audioType("MP3").artists(List.of(artist4)).build();
        Track track4 = Track.builder().id(nextId++).title("Lost Control").album("I've Tried Everything but Therapy").issueDate(LocalDate.of(2023,6,23)).durationSec(210).audioType("MP3").artists(List.of(artist5)).build();
        Track track5 = Track.builder().id(nextId++).title("Texas Hole 'Em").album("Cowboy Carter").issueDate(LocalDate.of(2024,2,11)).durationSec(239).audioType("MP3").artists(List.of(artist6)).build();
